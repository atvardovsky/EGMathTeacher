...
function FirstMeetingScreen({
  ...
  async function sendMeetingMessage(
    ...
    try {
      ...
      if (!displayPrompt || source === 'voice') {
        restorePromptAfterFailedSend(prompt, source);
      }
  ...
  function restorePromptAfterFailedSend(prompt: string, source: 'text' | 'voice') {
  ...
  function startVoice(auto = false) {
    ...
    const finishRecognition = () => {
      ...
      const retryAllowedForMode = auto ? voiceOutputEnabledRef.current : true;
      const canRetry =
        retryAllowedForMode &&
        speechSupported &&
...
function TutorWorkspace({
  ...
  const realtimePendingTurnsRef = useRef(new Map<string, string>());
  const realtimePendingPromptsRef = useRef(
    new Map<string, { prompt: string; source: 'text' | 'voice' }>(),
  );
  const realtimePendingTimeoutsRef = useRef(new Map<string, number>());
  ...
  function getOpenRealtimeLessonDataChannel(): RTCDataChannel | undefined {
    const channel = realtimeVoiceRef.current.dataChannel;
  ...
  function clearRealtimePendingRequest(requestId: string) {
    const timeout = realtimePendingTimeoutsRef.current.get(requestId);
    if (timeout) {
      window.clearTimeout(timeout);
      realtimePendingTimeoutsRef.current.delete(requestId);
    }
    realtimePendingTurnsRef.current.delete(requestId);
    realtimePendingPromptsRef.current.delete(requestId);
  }

  function clearAllRealtimePendingRequests(options: { restoreDraft?: boolean } = {}) {
    const pendingPrompt = realtimePendingPromptsRef.current.values().next().value;
    const pendingTurnIds = new Set(realtimePendingTurnsRef.current.values());
    for (const timeout of realtimePendingTimeoutsRef.current.values()) {
      window.clearTimeout(timeout);
    }
    realtimePendingTimeoutsRef.current.clear();
    realtimePendingTurnsRef.current.clear();
    realtimePendingPromptsRef.current.clear();
    if (options.restoreDraft && pendingPrompt) {
      restorePromptAfterFailedSend(pendingPrompt.prompt, pendingPrompt.source);
      setTurns((current) => current.filter((turn) => !pendingTurnIds.has(turn.id)));
    }
  }

  function sendMessageViaRealtimeDataChannel(
    prompt: string,
    source: 'text' | 'voice',
    ...
    setVoiceInterim('');
    setVoiceStatus(null);
    realtimePendingTurnsRef.current.set(requestId, id);
    realtimePendingPromptsRef.current.set(requestId, { prompt, source });
    const timeout = window.setTimeout(() => {
      const pending = realtimePendingPromptsRef.current.get(requestId) ?? { prompt, source };
      clearRealtimePendingRequest(requestId);
      setSending(false);
      setError(t.tutor.realtimeVoice.lessonTimeout);
      restorePromptAfterFailedSend(pending.prompt, pending.source);
      setTurns((current) => current.filter((turn) => turn.id !== id));
    }, REALTIME_LESSON_EVENT_TIMEOUT_MS);
    realtimePendingTimeoutsRef.current.set(requestId, timeout);
    setTurns((current) => [{ id, prompt, source, lessonType: currentLessonType }, ...current]);

    try {
      const sent = sendRealtimeLessonEvent({
        type: 'student_text',
        requestId,
        message: prompt,
      ...
      if (!sent) {
        throw new Error(t.tutor.realtimeVoice.dataChannelFailed);
      }
    } catch (err) {
      const pending = realtimePendingPromptsRef.current.get(requestId) ?? { prompt, source };
      clearRealtimePendingRequest(requestId);
      setSending(false);
      setError(err instanceof Error ? err.message : t.tutor.realtimeVoice.dataChannelFailed);
      restorePromptAfterFailedSend(pending.prompt, pending.source);
      setTurns((current) => current.filter((turn) => turn.id !== id));
    }
  }

  function handleRealtimeLessonEvent(event: WebRtcLessonServerEvent) {
    switch (event.type) {
      case 'session_ready':
        conversationIdRef.current = event.conversationId;
        setConversationId(event.conversationId);
        if (event.lessonSessionId) {
          setActiveLessonSessionId(event.lessonSessionId);
        }
        if (event.lessonType) {
          setLessonType(event.lessonType);
        }
        setRealtimeVoice((current) =>
          current.status === 'connected'
            ? { ...current, text: t.tutor.realtimeVoice.connectedWithLessonChannel }
            : current,
        );
        break;
      case 'heartbeat_ack':
      ...
      case 'tutor_answer': {
        ...
        const pendingTurnId = requestId
          ? realtimePendingTurnsRef.current.get(requestId)
          : undefined;
        if (requestId) {
          clearRealtimePendingRequest(requestId);
        }
        const turnId = pendingTurnId ?? event.turn.id;
        const answer = event.answer;
        const terminalAnswer = event.terminal || isTerminalTutorAnswer(answer);
        if (terminalAnswer) {
          conversationIdRef.current = undefined;
          setConversationId(undefined);
          setActiveLessonSessionId(undefined);
        ...
        setLessonType(event.lessonType);
        autoImageTurnIdsRef.current.add(answer.turnId ?? turnId);
        setTurns((current) => {
          const hasPending = current.some((turn) => turn.id === turnId);
          if (!hasPending) {
            return [{ ...event.turn, id: turnId, answer }, ...current];
          }
        ...
        if (terminalAnswer) {
          void stopRealtimeVoice(false, t.tutor.realtimeVoice.closedWithLessonTurn);
        }
        break;
      }
      case 'error': {
        const pendingTurnId = event.requestId
          ? realtimePendingTurnsRef.current.get(event.requestId)
          : undefined;
        const pendingPrompt = event.requestId
          ? realtimePendingPromptsRef.current.get(event.requestId)
          : undefined;
        if (event.requestId) {
          clearRealtimePendingRequest(event.requestId);
        }
        if (pendingTurnId) {
  ...
  function registerRealtimeLessonDataChannel(
    channel: RTCDataChannel,
    bootstrap: WebRtcBootstrapPayload,
  ) {
    channel.onopen = () => {
      ...
      sendRealtimeLessonEvent({ type: 'client_ready' });
    };
    channel.onclose = () => {
      if (realtimeVoiceRef.current.dataChannel === channel) {
        realtimeVoiceRef.current.dataChannel = undefined;
      }
    };
    channel.onerror = () => {
      if (realtimeVoiceRef.current.sessionId === bootstrap.sessionId) {
        setRealtimeVoice((current) =>
          current.status === 'connected'
            ? { ...current, text: t.tutor.realtimeVoice.dataChannelFailed }
            : current,
        );
      }
    };
    channel.onmessage = (event) => {
      try {
        const payload = JSON.parse(String(event.data)) as WebRtcLessonServerEvent;
        handleRealtimeLessonEvent(payload);
      } catch {
        setError(t.tutor.realtimeVoice.dataChannelFailed);
      }
    };
  }
  ...
  async function startRealtimeVoice() {
    ...
    try {
      ...
      dataChannel = peerConnection.createDataChannel('lesson-events', { ordered: true });
      registerRealtimeLessonDataChannel(dataChannel, bootstrap);
  ...
  async function stopRealtimeVoice(showStopped = true, statusText?: string) {
    ...
    clearAllRealtimePendingRequests({ restoreDraft: showStopped || Boolean(statusText) });
    setSending(false);
  ...
  async function generateImage(turn: TutorTurn, block: TutorImageBlock) {
    if (turn.loadingImages?.[block.id] || viewingHistoryRecord) {
      return;
    }
    setTurns((current) =>
      current.map((item) =>
    ...
    try {
      const result = await api<TutorImageResult>('/tutor/image', {
        ...
        body: JSON.stringify({
          prompt: block.prompt,
          context: buildImageGenerationContext(turn, block),
...
function getTutorBlocks(answer: TutorAnswer, imageAlt: string): TutorResponseBlock[] {
  if (Array.isArray(answer.blocks) && answer.blocks.length > 0) {
    ...
    if (!hasImageBlock && answer.needsImage) {
      blocks.push({
        id: 'image-1',
        type: 'image',
        status: 'suggested',
        ...
        priority: answer.imagePrompt ? 'optional' : 'important',
      });
    }
    return blocks;
  }
...
function imageGenerationKey(turn: TutorTurn, block: TutorImageBlock): string {
  return `${turn.answer?.turnId ?? turn.id}:${block.id}:${
    block.prompt ?? block.caption ?? block.altText ?? 'context'
  }`;
}

function buildImageGenerationContext(turn: TutorTurn, block: TutorImageBlock): string {
  const answer = turn.answer;
  if (!answer) {
    return [turn.prompt, block.caption, block.altText].filter(Boolean).join('\n');
  }
  const taskContext = answer.tasks
    .map((task) =>
      [`Задача: ${task.title}`, `Условие: ${task.prompt}`, task.difficulty ? `Сложность: ${task.difficulty}` : undefined]
        .filter(Boolean)
        .join('\n'),
    )
    .join('\n');
  const exampleContext = answer.examples
    .map((example) => [`Пример: ${example.title}`, example.explanation].join('\n'))
    .join('\n');
  return [
    turn.prompt ? `Запрос ученика: ${turn.prompt}` : undefined,
    answer.answer ? `Ответ репетитора: ${answer.answer}` : undefined,
    block.caption ? `Подпись изображения: ${block.caption}` : undefined,
    block.altText ? `Описание изображения: ${block.altText}` : undefined,
    taskContext,
    exampleContext,
  ]
    .filter(Boolean)
...
function cleanupRealtimeVoiceRuntime(runtime: RealtimeVoiceRuntime): void {
  ...
  const dataChannel = runtime.dataChannel;
  ...
  if (dataChannel) {
    ...
    if (dataChannel.readyState === 'open' || dataChannel.readyState === 'connecting') {
      try {
        ...
        // Local teardown should not block closing the lesson UI.
      }
    }
  }
